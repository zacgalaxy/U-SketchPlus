...
class convolutional_block(nn.Module):
    ...
    def forward(self, inputs):
        x = self.conv1(inputs) # first convolutional layer
...
class encoder_block(nn.Module):
    ...
    def forward(self, inputs):
        x = self.conv(inputs) # convolutional block forward method, used python dunder method (__call__)
        p = self.pool(x) # max of each layer is found and processed into 2x2 grid
        return x, p
...
class decoder_block(nn.Module):
    ...
    def forward(self, inputs, skip):
        x = self.up(inputs) # upsample the input with transpose convolution
        x = torch.cat([x, skip], axis=1)
        x = self.conv(x)
        return x
        

"""need to reverse this to make it a generator of sketches and not take in sketches"""
